...
public class CreateMutiChannel {
	private static Logger log = Logger.getLogger(CreateMutiChannel.class.getClass());
	...
	public static void main(String[] args) {
		try {
			...
			File pkFolder1 = new File(Config.ORG1_USR_ADMIN_PK);//组织1中的/keystore
			File[] pkFiles1 = pkFolder1.listFiles();
			File certFolder1 = new File(Config.ORG1_USR_ADMIN_CERT);//组织1中的/admincerts
			...
			// Create 5 new channel
			for(int i=0; i<5; i++) {
//				Orderer orderer = fabClient.getInstance().newOrderer(Config.ORDERER_NAME, Config.ORDERER_URL);
				ChannelConfiguration channelConfiguration = new ChannelConfiguration(new File(Config.CHANNELS_CONFIG_PATH[i]));
	
				byte[] channelConfigurationSignatures = fabClient.getInstance()
						.getChannelConfigurationSignature(channelConfiguration, org1Admin);
	
				Channel mychannel = fabClient.getInstance().newChannel(Config.CHANNELS_NAME[i], orderer, channelConfiguration,
...
//				Peer peer1_org2 = fabClient.getInstance().newPeer(Config.ORG2_PEER_1, Config.ORG2_PEER_1_URL);
				...
				mychannel = fabClient.getInstance().getChannel("mychannel1");
	            ...
	            while (peerIter.hasNext())
	            {
	            	  ...
	            	  log.info("Peer Name:"+pr.getName()+ ",  Peer Url:" + pr.getUrl());
	            }
			}// for 5
		} catch (Exception e) {
			e.printStackTrace();
